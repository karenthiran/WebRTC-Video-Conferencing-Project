...
muteBtn.addEventListener('click', () => {
    ...
    webrtc.localStream.getAudioTracks()[0].enabled = audioEnabled;
    muteBtn.textContent = audioEnabled ? 'Mute' : 'Unmute';
});

cameraBtn.addEventListener('click', () => {
    videoEnabled = !videoEnabled;
    webrtc.localStream.getVideoTracks()[0].enabled = videoEnabled;
    cameraBtn.textContent = videoEnabled ? 'Turn Off Camera' : 'Turn On Camera';
});
...
function sendMessage() {
    ...
    socket.emit('chat message', { message, room: webrtc.roomId });
